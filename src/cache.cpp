...
Cache::Cache(Option& op,uint8_t cachetype):cache_type(cachetype), total_size(0),option(op) {
    if(cachetype==CACHE_TYPE_BLOCKCACHE) max_size=op.BLOCKCACHE_SIZE;
    else if(cachetype==CACHE_TYPE_TABLECACHE) max_size=op.TABLECACHE_SIZE;
    else return;
}
...
CacheMem* Cache::Put(uint64_t key, uint64_t key2, char* pointer, size_t size){
    if(size>max_size) {
        std::cerr << "Cache Size Too Small! " << std::endl;
        exit(-1);
    }
